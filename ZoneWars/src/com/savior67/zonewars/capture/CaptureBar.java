...
//See http://forums.bukkit.org/threads/tutorial-utilizing-the-boss-health-bar.158018/
//for 1.7 update

public class CaptureBar {
	public static final int ENTITY_ID = 6000;
	...
	public static PacketPlayOutSpawnEntityLiving getMobPacket(String text, Location loc, int health){
		...
		try {
			...
			d.set(mobPacket, (int) Math.floor(loc.getBlockY()-300 * 32.0D));
...
//		mobPacket.d = (int) Math.floor(loc.getBlockY()-300 * 32.0D); //Y position
...
//		mobPacket.k = (short) 0; //Z velocity
	...
	public static DataWatcher getWatcher(String text, int health){
		text = colorize(text); //color it!
		text = text.substring(0, text.length()); //cut off the quotes
	...
	public static void displayTextBar(String text, final Player player, int health){
		...
		}.runTaskLater(ZoneWars.getInstance(), 120L);
	}
	
	public static void displayTextBarForAll(String text, int health){
		for(Player p:Bukkit.getOnlinePlayers()) {
			displayTextBar(text, p, health);
		}
	}
		...
		new BukkitRunnable(){
			...
			public void run(){
				if((loadUp ? health < 300 : health > 0)){
					...
					}.runTaskLater(ZoneWars.getInstance(), 40L);
					
					this.cancel();
				}
			}
		}.runTaskTimer(ZoneWars.getInstance(), delay, delay);
	}
	...
	public static void removeBar(final Player player) {
		...
		}.runTask(ZoneWars.getInstance());//.runTaskLater(ZoneWars.getInstance(), 40L);
	}
